...
export class JobsService {
  ...
  async getFailedJobs(workspaceId: string) {
    const deadLetteredRuns = await db
      .select()
      .from(jobRuns)
      .where(and(eq(jobRuns.workspaceId, workspaceId), eq(jobRuns.state, 'dead_lettered')));
  ...
  async replayFailedJob(workspaceId: string, jobRunId: string) {
    ...
    if (!queue || !this.isSupportedJob(failedRun.queueName, failedRun.jobName)) {
    ...
    const envelope = createJobEnvelope(failedRun.jobName, [workspaceId, failedRun.id, randomUUID()]);
    const replayPayload = {
      ...payload,
      ...envelope,
